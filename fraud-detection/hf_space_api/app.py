...
def download_models():
    ...
    for f in files:
        dest = MODELS_DIR / f
        if dest.exists():
            logger.info(f"Already exists: {f}")
            continue
        logger.info(f"Downloading {f} from HuggingFace...")
...
async def predict(transaction: TransactionRequest):
    ...
    try:
        ...
        proba = float(model.predict_proba(features_proc)[0][1])
        is_fraud = proba >= threshold

        explainer = artifacts["explainer"]
        shap_vals = explainer.shap_values(features_proc)
        impact = pd.Series(
            shap_vals[0], index=features_proc.columns
